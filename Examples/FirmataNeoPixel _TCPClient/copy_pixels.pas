...
begin
  ...
  Try
    ...
    Valid:=True;
    if (Value < 0) or (Value >= FormNeo1.Neopixel1.pixelsNumber) then
      Valid:=false;
  finally
    if not Valid then
    begin
...
begin
  ...
  Try
    ...
    if (Value < 0) or (value > 255) or (Value > FormNeo1.Neopixel1.pixelsNumber) then
      Valid:=false;
    if (strtoint(pixelsrc.text) + Value > FormNeo1.Neopixel1.pixelsNumber) then
      valid:=false;
    if (strtoint(pixeldest.text) + Value > FormNeo1.Neopixel1.pixelsNumber) then
      valid:=false;
  finally
    if not Valid then
    begin
...
begin
  FormNeo1.neopixel1.CopyPixels(strtoint(pixelsrc.text), strtoint(Pixeldest.text), strtoint(Pixelcount.text), ShowPixel.Checked);  // shift config
  Close;
end;
