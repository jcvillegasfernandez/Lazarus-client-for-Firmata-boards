...
#define IS_PIN_DIGITAL(p)       (((p) == 0 || (p) == 2 || (p) == 4 || (p) == 5) || ((p) >= 12 && (p) < A0))
...
#define digitalPinHasSerial(p)  ((p) == 16 || (p) == 17)
// Pins 6 to 11 SPI internal Flash
// Pins 4, 12 to 33 secure general use
// Pins 34, 35, 36, 39 only Input Pin
// Pin 12 boot fails if pulled high
// Pin 5 outputs PWM signal at boot, strapping pin
// Pin 3 RX pin, Input but HIGH at boot
// Pin 2 board led, must be left floating or LOW to enter flashing mode
// Pin 1 TX pin, Output, debug output at boot
// Pin 0 output-input, outputs PWM signal at boot, must be LOW to enter flashing mode
